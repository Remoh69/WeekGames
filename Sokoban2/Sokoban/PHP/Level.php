<?PHP
	...
	if($_GET["Requete"] == "Add_Level"){
		...
		$query = "Delete FROM Sokoban where Level = '" . $_GET["Level"] . "' Order By Level";
		...
		if ($Existe[0] == 0){
			...
			if ($Existe[0] == 0){
				...
				$query = 'INSERT INTO Utilisateurs_Sokoban (Login,Password,Email,Level) VALUES ("' . $_GET["Login"] . '","' . $_GET["Pass"] . '","' . $_GET["Mail"] . '",1)';
		...
		if ($Existe[0] == 1){
			...
			if($row->Password == $_GET["Pass"])
			{
				...
				echo 'Ok_' . $row->Login . '_' . $row->Level;
